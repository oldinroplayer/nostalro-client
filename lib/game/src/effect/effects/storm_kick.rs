...
// The original game's effect literals are in world units 1:1 with ours (effect
// dimensions add onto actor positions in the same GND-zoom world space), so the
// source values (`max_height = 40` radius, `height[0] = 50`, `height[1] = 5`,
// `distance = 2`) are used directly to match the original client's vortex size.
// One factor still preserves every source *ratio* (radius decay, height step).
