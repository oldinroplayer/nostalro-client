...
/// `PP_SLASH1` blade burst on this texture instead of `magic_blue`.
...
/// The SuperAngel (Angel2/Angel3) level-up ring — same eight-blade `PP_SLASH1`
/// burst (`flag1[0]==0`) as Kaizel, on `ring_blue.tga`. Spawned at frame 65.
...
/// `EF_STOPEFFECT` — `RunStop(0)` + `RunStop(45)`: the same eight-blade
/// `PP_SLASH1` star as Kaizel but the `flag1[0]==1` branch — blades start flat
/// (`max_height` 0), grow a shorter lift (cap 2, `+0.2`/frame), stretch slower
/// (`distance +1`/frame) and ramp alpha over the first 5 frames.
...
mod tests {
    ...
    fn stopeffect_starts_flat_and_grows_lift() {
        // RunStop blades start with no vertical lift (max_height 0) and grow it
        // in, unlike Kaizel which starts lifted. Same eight-blade star.
