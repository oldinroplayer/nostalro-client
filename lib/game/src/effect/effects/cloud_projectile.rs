...
/// (`vecB_now -= height[1]*15*(cos,sin)`), then flies inward through the centre.
...
/// burst looks random (the original `RotStart = random(360)`) while staying
...
impl CloudProjectileEffect {
    ...
    /// (`RotStart = random(360)`), starts far out in that direction, and flies
